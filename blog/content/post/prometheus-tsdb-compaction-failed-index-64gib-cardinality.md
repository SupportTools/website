...
First, the target block ULID (`01KY19HXKYAK3C86PT8KD98AEA`) with the `.tmp-for-creation` suffix is a block that never existed. It was being built. It is not a block you lost.
...
The practical consequence: **there is no flag for this.** Adding memory does not help. Faster disks do not help. Upgrading does not help — the limit is unchanged from 3.x through current `main`, and lifting it requires a varint index format that has not landed. If a merge would produce an index over 64 GiB, that merge is impossible on this Prometheus, today.
...
5. **Symbol-table size.** Many unique label *names* and *values* — high-entropy strings — enlarge the symbol table directly.
...
- `pod`, `pod_uid`, `container_id`, and ephemeral `instance` values or pod IPs — every rollout mints a fresh set
...
Use the `promtool` binary matching your Prometheus version — index format details differ across major versions.

```bash
DB=/var/lib/prometheus/metrics2

blocks=(
  01KWYDQGNSB9F3SQ8T1WRVD7ZP
  01KXFSZFV0NANWCQN30T8FYJ3Y
  01KY1655ENZYDZB22BN655XRDH
)
...
- `stats.numSeries` — the historical series count per block, the number that actually matters
...
- `compaction.failed` — confirms the blocks are now marked and being skipped
...
`--limit` controls how many entries appear in each list (default 20). Start **without** `--extended` — extended analysis on blocks this size drives substantial I/O and memory pressure, and you are already in a capacity incident. Run one block at a time, and prefer a snapshot or a copy on a machine that isn't serving queries.
...
Keep the caveat in mind: **these are head statistics.** They tell you what is being ingested now. They systematically understate the historical churn that fills block indexes. Use them to find what to fix going forward; use `promtool tsdb analyze` to understand what already happened.
...
Be honest about what this buys you. The flag is registered `Hidden()` with the help text *"Maximum duration compacted blocks may span. For use in testing. (Defaults to 10% of the retention period.)"* — upstream does not consider it a production tuning knob. It requires a restart. It prevents *future* oversized merges; it does not repair or split the blocks that already failed. And smaller maximum blocks mean more blocks per query, which costs you query performance.
...
- **Functional sharding** — split by cluster, environment, region, or service group
- **Hash-based target sharding** — distribute scrape targets across multiple Prometheus instances
- **Shorter local retention**, with the long tail living elsewhere
- **Remote write** to horizontally scalable storage — Thanos, Mimir, Cortex, VictoriaMetrics. See [Cortex Multi-Tenant Monitoring: Horizontally Scalable Prometheus as a Service](/cortex-multi-tenant-monitoring-production/) for one way to build that tier
- **Reducing churn at the source** — instrumentation and service discovery, not just relabeling at the edge
